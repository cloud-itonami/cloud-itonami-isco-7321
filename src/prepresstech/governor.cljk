(ns prepresstech.governor
  "PrepressTechGovernor — the independent safety/scope layer gating
  every workshop scheduling/logistics proposal an advisor may make for
  a pre-press shop crew. The governor never dispatches hardware
  itself, never performs plate-preparation or plate-processing work on
  the shop floor, and never finalizes a plate-preparation-execution
  decision (e.g. deciding to proceed with a specific plate-preparation
  or plate-processing procedure) or overrides a shop safety officer's
  judgment — those are permanently out of this actor's scope and
  remain a shop safety officer's exclusive judgment (README's
  'Robotics premise': this actor coordinates SHOP
  SCHEDULING/LOGISTICS ONLY — it never performs plate-preparation or
  plate-processing work itself). Modeled on cloud-itonami-isco-7311's
  instrumentmaker.governor for the physical-safety-domain shape.
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs plate-preparation or
                                plate-processing work itself; it only
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                plate-preparation-execution decision
                                (e.g. deciding to proceed with a
                                specific plate-preparation or
                                plate-processing procedure), or to
                                ...
                                \"authorize the plate processing
                                operation\" / \"override the shop
                                safety officer's judgment\", never as
                                bare nouns like \"plate\",
                                \"chemical\" or \"safety\", so the
                                ...
                                plate-preparation task …\" or \"…routed
                                for shop safety officer review\" — all
...
(def ^:private scope-excluded-ops
  #{:finalize-plate-preparation-decision :authorize-plate-processing-operation
    :proceed-with-plate-preparation-procedure :override-safety-officer-judgment
    :override-shop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("plate", "chemical", "processing", "safety", "shop safety
...
;; nouns, e.g. "plate-preparation task" / "shop safety officer
;; review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the plate-preparation operation" "proceed with the plate preparation operation"
   "proceed with the plate-preparation procedure" "proceed with the plate processing procedure"
   "finalize the plate-preparation decision" "finalize the plate processing decision"
   "authorize the plate-preparation operation" "authorize the plate processing operation"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "刷版準備・製版処理の実行判断の確定・shop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `prepresstech.store/Store`. Pure — never
