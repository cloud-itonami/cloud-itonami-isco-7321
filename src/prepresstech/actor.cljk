(ns prepresstech.actor
  "PrepressTechActor — the ISCO-08 7321 pre-press workshop
  ...
  resume after human sign-off. Modeled on cloud-itonami-isco-7311's
  instrumentmaker.actor for the physical-safety-domain shape.
  ...
  The unconditional invariant: the Pre-press Technician Advisor can
  never directly commit an operating record the PrepressTechGovernor
  refuses, and can never make this actor finalize a
  plate-preparation-execution decision (e.g. a specific
  plate-preparation or plate-processing procedure) or override a shop
  ...
  regardless of disposition path. This actor coordinates SHOP
  SCHEDULING/LOGISTICS ONLY — it never performs plate-preparation or
  plate-processing work itself."
...
(defn build-graph
  "Build a compiled PrepressTechActor graph. `store` implements
  `prepresstech.store/Store`. `advisor` implements
  `prepresstech.advisor/Advisor` (defaults to `mock-advisor`).
